...
| 技术 | 解决的问题 | 效果 |
|------|-----------|------|
| **KL Observer** | vtransform `bev_pool` 空间分布极度稀疏（类似one-hot）、统计直方图在零点处形成尖锐峰值导致 98.3% range waste | −12.6% → −0.5%（+12.1 pts） |
| **Log2 对数域量化** | lidar 稀疏激活呈零均值拉普拉斯分布，线性量化零点附近浪费 | −18.5% → −3.1%（+15.4 pts） |
