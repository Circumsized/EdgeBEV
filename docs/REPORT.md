...
本节给出报告中两项核心算法的完整数学形式。符号约定：**x** 为浮点激活，**x̂** 为反量化后的近似值，**b** 为位宽（本工作 b = 8）。
...
其**绝对误差有上界** |x̂ − x| ≤ s/2，与 x 的取值无关。但**相对误差**为

```text
   |x̂ − x| / |x|  ≤  (s/2) / |x|  →  ∞      (当 |x| → 0)
```

即：均匀量化在零点附近相对误差发散。对于零均值拉普拉斯分布的稀疏激活（大部分值趋近 0），这意味着**大量小值信息的相对损失极大**。此外，s ∝ max(|x|) 表明单个离群值会立刻放大步长、压缩主体的可用级别——这正是 vtransform 出现 98.3% range waste 的数学根因。

#### 3.5.2 KL 散度校准的推导

设校准得到的 N-bin 直方图为 hist。对候选截断位置 i（保留前 i 个 bin），先构造**截断参考分布** P^(i)——把超出阈值的概率质量 clip 到边界 bin：
...
[start_k, end_k] = [floor(k·i/M), floor((k+1)·i/M) − 1]，段长 L_k = end_k − start_k + 1。
把每段概率质量**均匀展开**回细粒度 bin，得到重分布：

```text
   Q̃_j^(i) = ( 1 / L_k ) · Σ_{t=start_k..end_k} P_t^(i),   其中 k = floor( j·M / i )
```

最后在所有候选 i 中选取使 KL 散度最小者：

```text
   i* = argmin_{i ∈ [M, N]}  Σ_{j=0..i-1}  P_j^(i) · log( P_j^(i) / Q̃_j^(i) )
      = argmin_i  D_KL( P^(i) ‖ Q̃^(i) )
   T  = bin_width · i*
```

之后以 T 替代 max(|x|) 计算 s。**语义**：D_KL 度量"用量化后分布 Q̃ 近似真实分布 P 所付出的额外信息代价"，最小化它等价于寻找在量化分辨率约束下信息保真度最高的截断点。
...
```text
   q = clamp( round( log2(|x|) − β ),  −2^(b−1)+1,  2^(b−1)−1 )
   x̂ = sign(x) · 2^( q + β )
```

其中基准 β 由非零激活的 p-分位数（本工作 p = 0.05）估计，使有效动态范围对齐整数格点：

```text
   β = quantile_p( { log2|x_i| : |x_i| > ε } )
```

**相对误差有界性**：设 q* = log2|x| − β 为未取整的理想值。因 round(·) 的取整误差 |Δ| ≤ 1/2，反量化后的比值为

```text
   x̂ / x = 2^(q − q*) = 2^Δ  ∈  [ 2^(−1/2),  2^(1/2) ]
...
**直通估计器（STE）**：反向传播时对 `round`/`clamp` 采用 STE，梯度 ∂x̂/∂x = 1（在非截断区间内），保证量化感知训练/微调时梯度可传。
...
**反直觉结论**：更细的粒度反而更差。原因是本工作校准集仅 ~128 batch，per-channel 需为每层每通道估计独立的 log2 基准（21 层 × C 通道），**参数量远超数据支撑能力，导致过拟合校准分布**，泛化到验证集时反而劣化。这是"模型容量 vs 数据量"权衡在量化中的体现。
...
```text
   y = sign(x) · 2^( round( log2(|x| + 1) / scale ) · scale )
```
...
2. **KL 散度是"最短有效描述"的直接度量**。公式 `D_KL(P ‖ Q) = H(P,Q) − H(P)`，
