...
2. **Phase 8**：`TVSparseEncoder` 替代 PyTorch `SparseEncoder23`，基于 `tv.Tensor` + `core_cc` API 实现完整 LiDAR backbone forward
3. **Phase 9 Part A**：TV backbone INT8 Log2 量化 —— 加载 PTQ 量化 weight 与 `log2_base`，手写 CUDA kernel 实现 Log2 / BN，对齐 FakeQuant 计算图

**部署验证结果（完整验证集 6019 帧）**：

| 路径 | LiDAR backbone | 量化 | NDS | mAP | 备注 |
|------|---------------|------|-----|-----|------|
...
**关键发现**：
1. **TV FP16 与 PyTorch FP16 NDS 一致**（0.7039 vs 0.7040），证明 `tv.Tensor` + `implicit_gemm` 数值正确
2. **PyTorch INT8 控制组 NDS = 0.6893**，验证 spconv 2.3 环境下 PTQ checkpoint 无 regression
3. **TV INT8 与 PyTorch INT8 NDS 完全一致**（0.6893），mAP 仅差 0.0004，属于 INT8 不同 kernel 实现的正常 rounding 差异
4. **关键 bug 修复**：`TVSparseEncoder._basic_block` 中 `identity = x.features.clone()`，修复 in-place Log2 量化破坏残差连接的问题（修复前 dense diff = 16.8，修复后 = 2.89）

**项目状态**：
- ✅ Phase 7/8/9 Part A 全部完成
- ✅ TV INT8 Log2（去 PyTorch LiDAR）精度达标
- ⏳ Phase 9 Part B（完全零 PyTorch，Jetson Orin）待设备到位后推进
